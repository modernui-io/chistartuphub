...
import { X, Download, Mail, Loader2, CheckCircle, FileSpreadsheet } from "lucide-react";
import { supabase } from "@/api/supabaseClient";
import { toast } from "sonner";

const CSV_URL = "/downloads/top-100-accelerators.csv";

/**
 * DownloadAcceleratorListModal - Lead Magnet for Top 100 Accelerators
 * Bureau Design System | Systematic Modernism
 */
export default function DownloadAcceleratorListModal({ isOpen, onClose }) {
  ...
  const triggerDownload = () => {
    ...
    link.href = CSV_URL;
    link.download = 'Top-100-Accelerators-ChiStartupHub.csv';
            ...
            <div className="p-4 md:p-6 space-y-4">
              <div className="border border-white/10 p-6 text-center">
                ...
                <p className="font-mono text-[10px] text-white/40 tracking-[0.02em] mb-6">
                  Check your downloads folder for the CSV file. It includes rankings, investment terms, acceptance rates, and more.
                </p>
                <button
              ...
              <div className="flex-1 pr-2">
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  <div className="w-6 h-6 border border-white/20 flex items-center justify-center">
                    <FileSpreadsheet className="w-3 h-3 text-white/50" strokeWidth={1.5} />
                  </div>
                  <span className="font-mono text-[11px] uppercase tracking-[0.1em] text-white">
                    Top 100 Accelerators
                  </span>
                ...
                <p className="font-mono text-[10px] text-white/40 uppercase tracking-[0.05em]">
                  The definitive list — rankings, terms, acceptance rates & more
                </p>
              </div>
              <button
                onClick={handleClose}
              ...
              <div className="border border-white/10 p-4 mb-6">
                ...
                <ul className="space-y-2">
                  <li className="flex items-center gap-2 text-white/60 text-sm">
                    ...
                    100 accelerators ranked by track record
                  </li>
                  <li className="flex items-center gap-2 text-white/60 text-sm">
                    <span className="text-white/30">—</span>
                    Investment terms & equity stakes
                  </li>
                  <li className="flex items-center gap-2 text-white/60 text-sm">
                    <span className="text-white/30">—</span>
                    Acceptance rates & program fees
                  </li>
                  <li className="flex items-center gap-2 text-white/60 text-sm">
                    <span className="text-white/30">—</span>
                    Top alumni & exit counts
                  </li>
                </ul>
              </div>
